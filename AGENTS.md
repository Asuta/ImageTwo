...
- 轻量画布：`src/components/canvas/CanvasProjectsPage.jsx` 负责 Canvas 二级项目页、新建、搜索、进入、重命名和删除；`src/components/canvas/CanvasWorkspace.jsx` 负责单个全屏画布的图片/文本节点、框选、复制克隆、文本节点八方向缩放、可编辑连线、`@` 节点引用、导航辅助、上下文生成面板和经典历史图片抽屉等交互；图片节点按图片原始像素固定宽高与比例，不提供节点缩放或拉伸，选中图片后的顶部工具栏只保留真实节点操作，不提供仅追加固定提示词的伪功能按钮，图片标注从“更多”菜单进入；经典历史图片只在用户从底部历史抽屉拖入时加入画布，画布内发起的生成结果仍自动落位。`src/components/canvas/AnnotationEditor.jsx` 负责非破坏式图片标注，`src/lib/canvas-db.js` 负责多项目 IndexedDB、按 `canvasId` 隔离的布局/上传素材/标注持久化和旧单画布数据迁移。画布不包含视频生成或视频节点。
...
- 画布中已有内容的图片节点必须以图片加载后的 `naturalWidth` / `naturalHeight` 作为唯一宽高来源，画布世界坐标中的 1 单位对应图片 1 像素；不得再按固定上限适配、设置最小显示尺寸、显示图片缩放柄或允许拉伸。旧布局、历史生成、上传、复制和标注图片在加载后都要以节点中心不变的方式校正到当前图片的真实像素尺寸；空图像占位节点仍使用创建时的占位尺寸，生成结果加载后再切换为真实尺寸。文本节点仍可八方向调整文本框尺寸。
