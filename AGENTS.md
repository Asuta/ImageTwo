...
- 新账号初始额度由 `IMAGE2_SIGNUP_CREDITS` 控制，当前默认值为 `5`；单次生图成本由 `IMAGE2_GENERATION_COST_CREDITS` 控制，当前默认值为 `0.05`，表示每张图片扣 0.05 点；余额、预扣、失败返还、统计和前端展示统一保留最多两位小数。`GET /api/auth/me` 会同时返回 `generationCostCredits`，经典模式和 Canvas 必须基于该运行时单价与当前生成数量显示预计消耗，不得在前端硬编码单价。
