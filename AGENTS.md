...
- 可选生图模型集中在 `src/lib/image-models.js`：`gpt-image-2.5-flare`（默认）和 `gpt-image-2.5-sunburst`，前端与 Node 后端共用。经典模式、Canvas、历史重试均须显式提交任务模型，后端在预扣前校验，并在异步任务、上游文生图/编辑请求和历史中保留所选模型，不能被供应商默认值或上游内部模型别名覆盖。切换模型不改共享供应商；Canvas 的模型设置按 `canvasId` 保存。旧历史重新生成时回退 Flare；旧供应商 `gpt-image-2` / `gpt-image-2-codex` 读取时兼容 Flare，保留其他自定义供应商模型配置。
...
- Canvas 新生成任务必须携带 `canvasContext.canvasId`，自动补齐生成节点和原地替换清理仅作用于该任务所属项目；无归属的旧单画布任务只自动补齐到 `default-workspace`。自动创建的历史节点 ID 必须含 `canvasId`，避免同一历史图片在不同项目的布局覆盖彼此。各项目已保存或手动拖入的历史图片引用仍保留，不得因任务归属不同而删除。
...
- “使用标注图”会显式添加当前标注节点的 `@` 引用，使标注 Blob 进入下一次编辑请求；普通选择图片仍不得默认把自身作为参考。删除该引用会同步移除参考输入。
