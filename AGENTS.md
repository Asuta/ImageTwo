...
- 画布分支关系由节点 `parentIds` 和任务 `canvasContext.parentIds` 表达；分支续作结果默认落在来源节点右侧并绘制连线，不要根据节点坐标反推父子关系。节点连接锚点位于节点左右边缘外侧，拖动右侧吸附球时由 `connectionDraft` 表达跟随鼠标和磁性吸附状态；在空白处松手时，新节点的左侧吸附球必须与松手位置重合。
