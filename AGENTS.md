...
- 轻量画布：`src/components/canvas/CanvasWorkspace.jsx` 负责全屏画布、图片/文本节点、上下文生成面板和 AI 助手抽屉等交互，`src/lib/canvas-db.js` 负责独立 IndexedDB 布局与上传素材持久化；画布不包含视频生成或视频节点。
...
- 经典历史保存在浏览器 `image2-local-history` IndexedDB；画布布局和本地上传素材保存在独立的 `image2-canvas-workspace` IndexedDB。为覆盖刷新前的保存窗口，画布还会把不含 Blob 的布局、视口和设置写入 `localStorage` 兜底；生成图片节点只持久化 `taskId` / `imageId` 引用，不复制经典历史中的生成 Blob。
...
- 画布分支关系由节点 `parentIds` 和任务 `canvasContext.parentIds` 表达；分支续作结果默认落在来源节点右侧并绘制连线，不要根据节点坐标反推父子关系。
...
- 画布改动需要同时验证经典模式无回归，以及画布平移/缩放、上传、图像/文本节点、添加菜单、AI 助手抽屉、上下文生成面板、拖拽、节点缩放、选图参考、分支续作与连线、聚焦所选、撤销/重做、生成落位、双模式同步和刷新恢复。
