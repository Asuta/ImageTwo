...
- `ai-pixel`：文生图和参考图都走 AI Pixel 的 OpenAI 图片生成接口 `/v1/images/generations`，后台供应商地址可填写 `https://ai-pixel.online`。参考图通过 JSON 的 `image` 数组传入。
...
无参考图时 `image` 为空数组。后台地址如果填写根地址 `https://ai-pixel.online`，服务端会自动追加 `/v1/images/generations`。如果前端选择 `auto` 比例，此格式会默认发送 `1024x1024`。AI Pixel 返回 `data[0].b64_json`，服务端会按现有图片结果逻辑解析。
