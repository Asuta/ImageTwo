...
- 轻量画布：侧栏可在经典模式与无限画布之间切换；画布采用全屏创作工作区，支持平移、缩放、框选多节点、图片上传、空图像节点、可编辑文本节点、八方向缩放、复制/粘贴/快速克隆、手动创建/选择/断开连线、从节点吸附球拖出新分支并自动继承上游图片/文本作为生成输入、在分支输入节点原地生成首张结果、`@` 引用图片或文本节点、图片标注与局部修改、导航地图、网格/连线显隐、快捷键提示、上下文生成面板、AI 创作助手抽屉、撤销/重做、生成结果自动落位和本地恢复。画布不提供视频生成或视频节点。
