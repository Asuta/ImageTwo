...
- 多项目无限画布：项目页支持空白新建、搜索、重命名、删除与长列表滚动，点击卡片进入独立工作区。工作区采用“素材 → 生成卡 → 结果组”：每张生成卡独立保存提示词、参考顺序、模型与参数；右侧固定面板区分草稿、素材详情与批量整理，选择不会改写生成输入。选图后“基于此图修改”只引用该图；连线、上传、画布选择、`@` 统一添加参考，文本仅经明确引用后参与生成。预览可查看实际提交正文，结果按批次放在生成卡右侧，保留视口与未提交草稿。标注生成新素材版本；查看结果可复用参数、只重试失败项、恢复原任务或按旧快照再生成一组。草稿与历史运行分离，移除节点仅隐藏布局，不丢引用或删除经典历史。
- Canvas 整理与导航：保留滚轮二维平移、Ctrl/⌘ 滚轮锚点缩放、空格/中键/右键平移、框选、复制粘贴克隆、八方向文本缩放、双击编辑文本、地图与撤销重做。图片使用标准尺寸且不可拉伸；素材右侧拖出创建生成卡，生成卡左侧拖出添加输入，历史来源线只读。素材抽屉包含项目素材、经典历史和运行记录。项目仅保存在当前浏览器，不提供云共享承诺；旧数据先备份再迁移，缺失素材显示错误，不能静默少传参考。详见 [画布交互与数据契约](docs/canvas-redesign.md)。
- Canvas 网页导航：Canvas 项目页与具体画布编辑页会写入独立的浏览器历史记录；进入画布后可使用浏览器后退返回此前页面，并可使用前进重新进入画布，刷新或直接打开带 `canvas` 参数的链接也会恢复对应层级。
- 双模式同步：画布生成继续使用原有任务、额度和历史链路；生成结果会同时出现在经典历史中。点击“使用标注图”会另存标注素材并创建引用它的新生成草稿，使标注图作为参考图提交；移除草稿中的对应引用即可取消，原图和旧运行保持不变。
...
- 经典模式记住最近选择；Canvas 每张生成卡独立保存模型，查看结果不覆盖草稿。历史“重新编辑 / 再次生成”和画布“复用生成参数 / 再生成一组”沿用记录中的可用模型，旧记录缺省为 Flare。
...
- `pnpm test`：启动隔离开发 API 和本地图片接口，检查并发归档、额度更新、任务归属、请求去重、重启退款与结果恢复、错误请求、礼品卡和双模型。 同时验证画布输入解析、不可变快照、缺失输入和旧数据迁移规则。
- `pnpm run test:browser`：启动 Vite 与隔离开发 API，用真实浏览器检查验证码登录、刷新/响应丢失恢复、多图账务、Canvas 独立草稿、输入快照、标注、部分失败重试/恢复、完整手势、迁移备份和历史保留。Windows 默认使用已安装的 Edge；其他系统默认使用 Playwright Chromium，可通过 `IMAGE2_TEST_BROWSER` 指定浏览器 channel，缺少 Chromium 时运行 `pnpm exec playwright install chromium`。
